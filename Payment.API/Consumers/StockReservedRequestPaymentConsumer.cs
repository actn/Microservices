...
{
    public class StockReservedRequestPaymentConsumer : IConsumer<IStockReservedeRequestPayment>
    {
        private readonly ILogger<StockReservedeRequestPayment> _logger;
        private readonly IPublishEndpoint _publishEndpoint;


        public StockReservedRequestPaymentConsumer(ILogger<StockReservedeRequestPayment> logger, IPublishEndpoint publishEndpoint)
        {
            _logger = logger;
            _publishEndpoint = publishEndpoint;
        }

        public async Task Consume(ConsumeContext<IStockReservedeRequestPayment> context)
        {
            var balance = 3000;
